...
public class SearchCheckoutRecordByBook {
	...
	private void defineMiddlePanel() {
		...
		JPanel addItemPanel1 = new JPanel(new FlowLayout(FlowLayout.LEFT, 15, 4));
		searchButton = new JButton("Search");
		addItemPanel1.add(searchButton);
	...
	private void sortTableModelByColumn(DefaultTableModel model, int columnIndex) {
		...
		for (int i = 0; i < model.getRowCount(); i++) {
			rows.add(new Object[] { model.getValueAt(i, 0), // Name column
					model.getValueAt(i, 1), model.getValueAt(i, 2), model.getValueAt(i, 3), model.getValueAt(i, 4),
					model.getValueAt(i, 5), model.getValueAt(i, 6)// Age column
			});
		}
